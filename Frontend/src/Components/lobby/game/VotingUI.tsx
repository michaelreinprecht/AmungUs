  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 rounded flex items-center justify-center p-4 overflow-auto">
      <div className="bg-black border-2 border-white rounded-xl shadow-2xl p-4 w-full max-w-6xl mx-auto">
        <h2 className="font-mono text-center text-2xl text-white font-bold mb-6">Who Is The Imposter?</h2>

        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 mb-4">
          {votingPlayerInfos.map((playerInfo) => (
            <div key={playerInfo.playerName} className={`flex flex-col items-center justify-center space-y-2 ${!playerInfo.alive && "opacity-50"}`}>
              <div className="text-white text-lg font-bold">{playerInfo.playerName}</div>
              <button
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={stopVoting}
            className="text-lg font-semibold py-2 px-4 border-2 border-gray-600 text-gray-300 rounded-lg transition-colors duration-200 ease-in-out hover:border-gray-400 hover:text-white"
          >
            Skip Voting
          </button>
        </div>
